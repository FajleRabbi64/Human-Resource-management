...
router.post("/forgotPassword", async (req, res) => {
  ...
  // Token will expire in 1 hour
    ...
    const resetLink = `http://localhost:3000/#/resetPassword`; // Update with your actual frontend URL

    const mailOptions = {
      from: "humanresourcesystem9@gmail.com",
      to: email,
...
// POST /resetPassword
// POST /resetPasswordWithoutToken
...
// ************************** SALARY PART: START ***********************************
// @desc: get list of emp's salalary receipts
...
router.get('/attendance-apply', async (req, res) => {
  try {
    const attendanceApplyRecords = await AttendanceApply.find({});
    res.json(attendanceApplyRecords);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
