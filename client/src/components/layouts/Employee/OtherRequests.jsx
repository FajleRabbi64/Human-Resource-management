...
export default class otherRequest extends Component {
  ...
  onBonusSubmit = async (user, e) => {
    ...
    const notePattern = /^[A-Za-z\s]+$/;

    // Validate bonusNote
    if (!notePattern.test(this.state.bonusNote)) {
      this.setState({ err: "Explain the circumstances must have string" }); // Update the error state
      return;
    }
  ...
  onLoanSubmit = async (user, e) => {
    ...
    // Validate loanNote
    if (!notePattern.test(this.state.loanNote)) {
      this.setState({ error: "Explain reason in brief must have string" }); // Update the error state
      return;
    }
